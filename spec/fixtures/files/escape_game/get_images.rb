...
def get_image_page_link(stage_name, filetype)
  "#{WEB_ROOT}/File:SSBU-#{URI::encode(stage_name.tr(" ", "_"))}.#{filetype}"
end

def get_direct_image_link(stage_name)
  @doc = Nokogiri::HTML(URI.open(get_image_page_link(stage_name, "png"))) do |config|
    config.nononet
  end
  return URI("#{WEB_ROOT}#{@doc.at_css("div.fullMedia a.internal").attributes["href"]}")
